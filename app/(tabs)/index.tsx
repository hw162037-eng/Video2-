...
export default function HomeScreen() {
  const { profiles, tasks, sharedImgbbKey, addTask } = useAppStore();
  ...
  const selectAudio = async () => {
    ...
    setAudioAssets((current) => [...current, ...picked.filter((asset) => !current.some((item) => item.uri === asset.uri))].slice(0, 3));
  };
  ...
  return (
    <ScreenContainer className="p-4" edges={["top", "left", "right"]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.headerRow}><View><Text style={styles.eyebrow}>AI MEDIA STUDIO</Text><Text style={styles.title}>Создать генерацию</Text></View></View>
        <Text style={styles.subtitle}>Выберите профиль, задачу и отправьте результат в очередь.</Text>
        ...
        <View style={styles.card}>
          <SectionTitle icon="account-circle" title="Профиль" />
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>{profiles.filter((profile) => profile.enabled).map((profile) => { const activeCount = tasks.filter((task) => task.profileId === profile.id && ["preparing", "submitting", "processing", "retry_wait"].includes(task.status)).length; return <Pressable key={profile.id} onPress={() => setSelectedProfileId(profile.id)} style={[styles.chip, selectedProfile?.id === profile.id && { backgroundColor: "#153A59", borderColor: blue }, activeCount > 0 && { borderColor: "#FF6B78", backgroundColor: "#3A202A" }]}><Text style={[styles.chipText, selectedProfile?.id === profile.id && { color: "#B9E1FF" }, activeCount > 0 && { color: "#FFB7BC" }]}>{profile.name}{activeCount > 0 ? ` · ${activeCount}` : ""}</Text></Pressable>; })}</ScrollView>
          {!profiles.some((profile) => profile.enabled) ? <Text style={styles.warningText}>Нет включённых профилей. Добавьте ключ в Настройках.</Text> : null}
        </View>
        ...
        {kind === "video" ? <View style={styles.card}>
          ...
          {videoMode === "reference_with_audio" && isFlash ? <><Pressable onPress={selectAudio} style={[styles.uploadBox, { borderColor: "#7454B8" }]}><MaterialIcons name="audiotrack" size={24} color={violet} /><Text style={styles.uploadTitle}>{audioAssets.length ? `${audioAssets.length} аудиофайла выбрано` : "Добавить аудио-референс"}</Text><Text style={styles.uploadHint}>MP3, WAV, AAC · максимум 3 · можно добавлять по одному</Text></Pressable>{audioAssets.length ? <View style={styles.audioList}>{audioAssets.map((asset) => <View key={asset.uri} style={styles.audioItem}><Text numberOfLines={1} style={styles.audioName}>{asset.name}</Text><Pressable onPress={() => setAudioAssets((current) => current.filter((item) => item.uri !== asset.uri))}><MaterialIcons name="close" size={16} color="#FF9AA0" /></Pressable></View>)}</View> : null}</> : null}
