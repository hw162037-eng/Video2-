...
import { Alert, FlatList, Image, Modal, Platform, Pressable, Share, StyleSheet, Text, TextInput, View } from "react-native";
...
function TaskCard({ task, now, developerMode, honestProgress, showTechnicalIds, showTimingBreakdown, onRetry, onCancel, onDelete, onPreview, onSave, onContinue }: { task: GenerationTask; now: number; developerMode: boolean; honestProgress: boolean; showTechnicalIds: boolean; showTimingBreakdown: boolean; onRetry: () => void; onCancel: () => void; onDelete: () => void; onPreview: () => void; onSave: () => void; onContinue: () => void }) {
  ...
  return <View style={styles.taskCard}>
    ...
    {showTimingBreakdown && (task.queueWaitMs !== undefined || task.serverGenerationMs !== undefined) ? <Text style={styles.taskSub}>Очередь: {formatTaskDuration(task.queueWaitMs ?? 0)} · Подготовка: {formatTaskDuration(task.prepareDurationMs ?? 0)} · Отправка: {formatTaskDuration(task.submitDurationMs ?? 0)} · Agnes: {formatTaskDuration(task.serverGenerationMs ?? 0)} · Polling: {formatTaskDuration(task.pollingDurationMs ?? 0)} · Всего: {formatTaskDuration(task.totalDurationMs ?? duration)}</Text> : null}
...
export default function TasksScreen() {
  ...
  const [continueAudioAssets, setContinueAudioAssets] = useState<import("@/lib/types").FileAsset[]>([]);
  useEffect(() => {
  ...
  const chooseContinueAudio = async () => {
    const result = await DocumentPicker.getDocumentAsync({ type: "audio/*", multiple: true, copyToCacheDirectory: true });
    if (result.canceled) return;
    const picked = result.assets.map((asset) => ({ uri: asset.uri, name: asset.name, mimeType: asset.mimeType, size: asset.size }));
    setContinueAudioAssets((current) => [...current, ...picked.filter((asset) => !current.some((item) => item.uri === asset.uri))].slice(0, 3));
  };
