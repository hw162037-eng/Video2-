...
import { extractLastFrame } from "@/lib/native-work-manager";
...
function TaskCard({ task, now, onRetry, onCancel, onDelete, onPreview, onSave, onContinue }: { task: GenerationTask; now: number; onRetry: () => void; onCancel: () => void; onDelete: () => void; onPreview: () => void; onSave: () => void; onContinue: () => void }) {
  ...
  return <View style={styles.taskCard}>
    ...
    <View style={styles.progressTrack}><View style={[styles.progressBar, { width: `${Math.max(2, task.progress)}%`, backgroundColor: color }]} /></View>
    <View style={styles.progressRow}><Text style={[styles.stageText, { color }]}>{task.stage}</Text><Text style={styles.percent}>{task.progress}%</Text></View>
...
export default function TasksScreen() {
  ...
  const { tasks, retryTask, cancelTask, removeTask, clearHistory, saveTaskResult, addTask } = useAppStore();
  ...
  const continueVideo = async (task: GenerationTask) => {
    const sourceUri = task.localUri || task.resultUrl;
    if (!sourceUri) {
      Alert.alert("Не найдено видео", "Для продолжения нужен сохранённый файл или ссылка на готовое видео.");
      return;
    }
    if (Platform.OS === "web") {
      Alert.alert("Continue Video", "В HTML-просмотре доступна только проверка интерфейса. Извлечение последнего кадра выполняется в Android APK через MediaMetadataRetriever после сохранения видео на устройстве.");
      return;
    }
    try {
      const frameUri = await extractLastFrame(sourceUri, task.id);
      if (!frameUri) throw new Error("Нативный модуль недоступен в этой сборке.");
      const now = Date.now();
      addTask({ ...task, id: `task-${now}-${Math.random().toString(36).slice(2, 7)}`, mode: "keyframe", prompt: task.prompt, status: "queued", stage: "В очереди на продолжение видео", progress: 0, createdAt: now, updatedAt: now, startedAt: undefined, completedAt: undefined, durationMs: undefined, serverId: undefined, resultUrl: undefined, localUri: undefined, errorCode: undefined, errorMessage: undefined, attempts: 0, firstFrame: undefined, lastFrame: { uri: frameUri, name: `last-frame-${task.id}.png`, mimeType: "image/png" } });
    } catch (error) {
      Alert.alert("Не удалось продолжить видео", error instanceof Error ? error.message : "Извлечение последнего кадра завершилось ошибкой.");
    }
  };
