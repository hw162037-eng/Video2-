...
class AgnesWorkManagerModule(private val reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
  ...
  fun getGenerationStatus(taskId: String, promise: Promise) {
    try {
      ...
      if (json.has("submittedAt")) result.putDouble("submittedAt", json.optLong("submittedAt").toDouble())
      if (json.has("pollingDurationMs")) result.putDouble("pollingDurationMs", json.optLong("pollingDurationMs").toDouble())
